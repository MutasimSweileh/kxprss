                                ...
                                <div class="col-lg-3 col-md-12" id="p<?= $data[$y]["id"] ?><?= ($_key == "wishlist" ? "" : "no") ?>">
                                    <div class="product">
                                        <div class="product-inner">
                                            <div class="product-card product-<?= ($data[$y]["discount"] ? "sale" : ($data[$y]["new"] ? "new" : "")) ?>"><?= $data[$y]["discount"] ? getValue("sale" . $plang) : ($data[$y]["new"] ? getValue("new" . $plang) : "") ?></div>
                                            <figure class="product-media">
                                                <a title="<?= $data[$y]["name" . $clang] ?>" href="<?= $core->getBlogUrl(array($data[$y]["id"], $data[$y]["name" . $clang]), "products") ?>" class="product-view"><img src="images/<?= $data[$y]["image"] ?>" alt="<?= $data[$y]["name" . $clang] ?>" class="img-responsive"></a>
                                                <div class="product-links">
                                                    <a href="#" class="add_to_wishlist wishlist <?= iswish($data[$y]["id"]) ?>" onclick="addToWishList(this,'<?= $data[$y]["id"] ?>'); return false;" data-id="<?= $data[$y]["id"] ?>"><i class="fas fa-heart"></i></a>
                                                </div>
                                            </figure>
                                            <div class="product-divider"></div>
                                            <div class="product-caption">
                                                <div class="product-name"><a href="<?= $core->getBlogUrl(array($data[$y]["id"], $data[$y]["name" . $clang]), "products") ?>"><?= $data[$y]["name" . $clang] ?>
                                                    </a></div>
                                                <div class="product-prices clearfix">
                                                    <?php $price = $data[$y]["price"];
                                                    if ($data[$y]["discount"] && !$expiry_date[0]) { ?>
                                                        <div class="product-price-old"> <?= plang("جنية", "L.E") ?> <?= $price ?> </div>
                                                    <?php $price = $data[$y]["discount"];
                                                    } ?>
                                                    <div class="product-price"> <?= plang("جنية", "L.E") ?> <?= $price ?></div>
                                                </div>
                                                <div class="product-add-to-cart"><a href="#" class="add_to_wishlist wishlist <?= expiry_date($data[$y]) ?> <?= $data[$i]["stock"] ? "" : "stock" ?> addtocart<?= $data[$y]["id"] ?>" onclick="addtocart('<?= $data[$y]["id"] ?>'); return false;" data-id="<?= $data[$y]["id"] ?>"><?= plang('اضافة للسلة', 'Add to
                                            Cart') ?></a></div>
                                            </div>
                                        </div>
                                    </div>
