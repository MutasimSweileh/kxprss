...
$myorder = array();
if ($del)
  $core->SqlDel("slider", "where id=" . $del);
if (!$add && !$edit) {
?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary" style="text-transform: capitalize;float: right;">السلايدر</h6>
      <div class="d-sm-flex align-items-center justify-content-between " style="float: left;">
        <div class="form-groupd col2">
          <a href="?app=<?= $app ?>&add=true" class="btn btn-success btn-icon-split">
            ...
            <span class="text">اضافة صورة</span>
          </a>
        </div>
      </div>
      <div class="clear"></div>
...
<?php } else {
  if ($edit)  $myorder = $core->getslider(array("id" => $edit));  ?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
    ...
    <div class="card-body">
      <form onsubmit="onsubmit(this); return false;" action="admin/?app=<?= $app ?>" id="<?= $app ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
          <div class="form-group col">
            <label for="exampleFormControlInput4">الوصف</label>
            <textarea type="text" class="form-control" data-noeditor id="exampleFormControlInput4" name="description">
           <?= $myorder[0]["description"] ?></textarea>
          </div>
          <div class="form-group col">
            <label for="exampleFormControlInput3">الوصف العربى</label>
            <textarea type="text" class="form-control" data-noeditor id="exampleFormControlInput3" name="description_arabic">
           <?= $myorder[0]["description_arabic"] ?></textarea>
          </div>
        </div>
        <div class="row">
          <div class="col">
            <div class="form-group">
              <label for="exampleFormControlInput1">الرابط</label>
              <input type="text" class="form-control" id="exampleFormControlInput1" name="link" value="<?= ($myorder[0]["link"] ? $myorder[0]["link"] : "") ?>">
            </div>
            <div class="form-group">
