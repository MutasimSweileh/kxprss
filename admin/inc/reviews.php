...
$myorder = array();
if ($del)
  $core->SqlDel("reviews", "where id=" . $del);
if (!$add && !$edit) {
?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary" style="text-transform: capitalize;float: right;">الشركات</h6>
      <div class="d-sm-flex align-items-center justify-content-between " style="float: left;">
        <div class="form-groupd col2">
          <a href="?app=<?= $app ?>&add=true" class="btn btn-success btn-icon-split">
            ...
            <span class="text">اضافة تقييم</span>
          </a>
        </div>
      </div>
      <div class="clear"></div>
      ...
      <div class="table-responsive">
        <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            <?php
            $index = 1;
            $data =  $core->getData("reviews", array());
    ...
    border-radius: 50%;   width: 100px;" src="../images/<?= $data[$i]["image"] ? $data[$i]["image"] : "boss2.png" ?>" alt="" /></td>
                ...
                <td style="vertical-align: middle;direction: rtl;"><?= $data[$i]["txt"] ?></td>
                <td style="vertical-align: middle;direction: rtl;"><?= $data[$i]["stars"] ?></td>
...
<?php } else {
  if ($edit)  $myorder = $core->getData("reviews", "where id=" . $edit);  ?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
        ...
        <div class="row">
          ...
          <div class="form-group col">
            ...
            <select class="form-control" name="product_id">
              ...
              <?php $count = $core->getData("products");
              for ($i = 0; $i < count($count); $i++) { ?>
                <option data="<?= $count[$i]["id"] ?>" <?php if ($myorder[0]["product_id"] == $count[$i]["id"]) {  ?> selected="selected" <?php } ?> value="<?= $count[$i]["id"] ?>"><?= $count[$i]["name"] ?></option>
              <?php } ?>
            </select>
          </div>
        </div>
        <div class="row">
          <div class="col">
            <div class="form-group">
              <label for="exampleFormControlInput4">التقييم</label>
              <textarea type="text" class="form-control" data-noeditor id="exampleFormControlInput4" name="txt"><?= $myorder[0]["txt"] ?></textarea>
            </div>
            <div class="form-group">
              <label for="exampleFormControlInput1">النجوم</label>
              <input type="number" class="form-control" id="exampleFormControlInput1" name="stars" value="<?= ($myorder[0]["stars"] ? $myorder[0]["stars"] : 0) ?>">
            </div>
            <div class="form-group">
