...
$myorder = array();
if ($del)
  $core->SqlDel("ads", "where id=" . $del);
if (!$add && !$edit) {
?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary" style="text-transform: capitalize;float: right;">الاعلانات</h6>
      <div class="d-sm-flex align-items-center justify-content-between " style="float: left;">
        <div class="form-groupd col2">
          <a href="?app=<?= $app ?>&add=true" class="btn btn-success btn-icon-split">
            ...
            <span class="text">اضافة اعلان</span>
          </a>
        </div>
      </div>
      <div class="clear"></div>
    ...
    border-radius: 50%;   width: 100px;" src="../images/<?= $data[$i]["image"] ? $data[$i]["image"] : "boss2.png" ?>" alt="" /></td>
                <td style="vertical-align: middle;direction: rtl;"><?= $data[$i]["link"] ? $data[$i]["link"] : "لايوجد" ?></td>
                <td style="vertical-align: middle;direction: rtl;"><?= $core->getStrAds($data[$i]["selection"]) ?></td>
...
<?php } else {
  if ($edit)  $myorder = $core->getAds(array("id" => $edit));  ?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      ...
      <form onsubmit="onsubmit(this); return false;" action="admin/?app=<?= $app ?>" id="<?= $app ?>" method="post" enctype="multipart/form-data">
        <div class="row">
          ...
          <div class="form-group col">
            ...
            <input type="number" class="form-control" id="exampleFormControlInput1" name="selection" value="<?= $myorder[0]["selection"] ?>">
          </div>
        </div>
        <div class="row">
          <div class="col">
