...
$myorder = array();
if ($del)
  $core->SqlDel("zones", "where id=" . $del);
if (!$add && !$edit) {
?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary" style="text-transform: capitalize;float: right;">الاسعار</h6>
      <div class="d-sm-flex align-items-center justify-content-between " style="float: left;">
        <div class="form-groupd col2">
          <a href="?app=<?= $app ?>&add=true" class="btn btn-success btn-icon-split">
            ...
            <span class="text">اضافة سعر</span>
          </a>
        </div>
      </div>
      <div class="clear"></div>
          ...
          <tbody>
            ...
            ?>
              <tr>
                ...
                <td style="vertical-align: middle;direction: rtl;"><?= $data[$i]["up"] ?></td>
                <td style="vertical-align: middle;direction: ltr;"><?= $data[$i]["add"] ?></td>
...
<?php } else {
  if ($edit)  $myorder = $core->getData("zones", "where id=" . $edit);  ?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary" style="text-transform: capitalize;"> اضافة او تعديل</h6>
    </div>
    <div class="card-body">
      <form onsubmit="onsubmit(this); return false;" action="admin/?app=<?= $app ?>" id="zones" method="post" enctype="multipart/form-data">
        <div class="row">
          <div class="form-group col">
            <label for="exampleFormControlInput1">حتى 2 كيلو</label>
            <input type="text" class="form-control" id="exampleFormControlInput1" name="up" value="<?= $myorder[0]["up"] ?>">
          </div>
          <div class="form-group col">
            <label for="exampleFormControlInput1">كيلو إضافي</label>
            <input type="text" class="form-control" id="exampleFormControlInput1" name="add" value="<?= str_replace('"', "'", $myorder[0]["add"]) ?>">
          </div>
          <div class="form-group col">
