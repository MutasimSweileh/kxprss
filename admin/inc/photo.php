...
$myorder = array();
if ($del)
  $core->SqlDel("photo", "where id=" . $del);
if (!$add && !$edit) {
?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      ...
      <div class="d-sm-flex align-items-center justify-content-between " style="float: left;">
        <div class="form-groupd col2">
          <a href="?app=<?= $app ?>&add=true&level=<?= $level ?>" class="btn btn-success btn-icon-split">
      ...
      <div class="table-responsive">
        <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            ...
            if ($level) {
              $data = array("category" => $level);
            }
            $data =  $core->getPhoto($data);
            for ($i = 0; $i < count($data); $i++) {
              $date = $core->getDateTime($data[$i]["date"], "arabic");
              // $login = $core->getspecialized(array("id"=>$data[$i]["specialized"]));
              $login = false;
              if ($data[$i]["category"])
                $login = $core->getPhoto(array("id" => $data[$i]["category"]));
              if ($data[$i]["category"] && !$level)
                continue;
            ?>
              <tr>
    ...
    border-radius: 50%;   width: 100px;" src="../images/<?= $data[$i]["image"] ? $data[$i]["image"] : "boss2.png" ?>" alt="" /></td>
                <td style="vertical-align: middle;direction: rtl;"><?= $login ? $login[0]["name"] : "تصنيف رئيسيى" ?></td>
                ...
                <td class="center" style=" text-align: center;vertical-align: middle;"><?php if ($glev) { ?><a href="?app=<?= $app ?>&del=<?= $data[$i]["id"] ?>" style="margin-left: 5px;" class="btn btn-danger btn-circle btn-sm">
                      <i class="fas fa-trash"></i>
                    </a><?php } ?><a href="?app=<?= $app ?>&edit=<?= $data[$i]["id"] ?>&level=<?= $level ?>" class="btn btn-warning btn-circle btn-sm">
                    <i class="fas fa-edit"></i>
                    <?php if (!$level) { ?>
                  </a><a href="?app=<?= $app ?>&level=<?= $data[$i]["id"] ?>" class="btn btn-primary btn-circle btn-sm">
...
<?php } else {
  if ($edit)  $myorder = $core->getPhoto(array("id" => $edit));  ?>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
        ...
        <div class="row">
          <div class="form-group col">
            ...
            <select class="form-control" name="category">
              ...
              $count = $core->getPhoto($data);
              for ($i = 0; $i < count($count); $i++) {
                if ($count[$i]["category"]  == 0) {
                  // continue;
              ?>
                  <option data="<?= $count[$i]["id"] ?>" <?php if ($myorder[0]["category"] == $count[$i]["id"] || $level ==  $count[$i]["id"]) {  ?> selected="selected" <?php } ?> value="<?= $count[$i]["id"] ?>"><?= $count[$i]["name"] ?></option>
              <?php }
              } ?>
            </select>
          </div>
